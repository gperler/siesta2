...
class SavePlugin extends BasePlugin
{
    ...
    const string METHOD_CREATE_SP_CALL_STATEMENT = "createSaveStoredProcedureCall";
    ...
    {
        if (count($this->entity->getCollectionList()) !== 0) {
            return true;
        }
        if (count($this->entity->getCollectionManyList()) !== 0) {
            return true;
        }
        if (count($this->entity->getDynamicCollectionList()) !== 0) {
            return true;
        }
        return false;
    }
    ...
    protected function generateCreateSPCall(): void
    {
        ...
        $line = 'return "CALL ' . $insertName . '(" . ' . implode(" . ',' . ", $escapeList) . " . ');';";
    ...
    protected function getQuoteCallList(GeneratorHelper $helper): array
    {
        ...
        foreach ($this->entity->getAttributeList() as $attribute) {
            ...
            $list[] = $helper->generateQuoteCall(
                $attribute->getPhpType(),
                $attribute->getDbType(),
                $memberName,
                $attribute->getIsObject(),
                $attribute->getLength(),
                $attribute->implementsArraySerializable(),
                $attribute->isEnum(),
            );
        }
        return $list;
