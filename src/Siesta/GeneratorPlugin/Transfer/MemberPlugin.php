...
class MemberPlugin extends BasePlugin
{
    ...
    protected function generateAttributeMember(): void
    {
        foreach ($this->entity->getAttributeList() as $attribute) {
            $this->classGenerator->addProtectedProperty(
                $attribute->getPhpName(),
                $attribute->getFullyQualifiedTypeName(),
            );
        }
    }
    ...
    protected function generateReferenceMember(): void
    {
        foreach ($this->entity->getReferenceList() as $reference) {
            ...
            $this->classGenerator->addProtectedProperty($reference->getName(), $foreignEntity->getTransferClassName());
        }
    }
    ...
    protected function generateCollectionMember(): void
    {
        foreach ($this->entity->getCollectionList() as $collection) {
            ...
            $this->classGenerator->addProtectedProperty($collection->getName(), $foreignEntity->getTransferClassName() . '[]');
        }
    }
    ...
    protected function generateCollectionManyMember(): void
    {
        foreach ($this->entity->getCollectionManyList() as $collectionMany) {
            ...
            $this->classGenerator->addProtectedProperty($collectionMany->getName(), $foreignEntity->getTransferClassName() . '[]', 'null');

            $mappingEntity = $collectionMany->getMappingEntity();
            $this->classGenerator->addProtectedProperty($collectionMany->getName() . 'Mapping', $mappingEntity->getTransferClassName() . '[]', 'null');
        }
    }
}