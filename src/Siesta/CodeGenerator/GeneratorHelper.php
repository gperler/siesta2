...
class GeneratorHelper
{
    ...
    {
        $this->method->addParameter($attribute->getPhpType(), $attribute->getPhpName(), $defaultValue, null, true);
    }
    ...
    public function addQuoteAttributeList(array $attributeList, bool $forceConnectionLookup = false, bool $fromMember = false): void
    {
        ...
        foreach ($attributeList as $attribute) {
            ...
            $quoteCall = $this->generateQuoteCall($attribute->getPhpType(), $attribute->getDbType(), $source, $attribute->getIsObject(), $attribute->getLength(), $attribute->implementsArraySerializable());
     ...
     * @return string
     */
    public function generateQuoteCall(string $phpType, ?string $dbType, string $variableName, bool $isObject = false, int $maxLength = null, bool $arraySerializable = false, bool $isEnum = false): string
    {
        ...
        if ($maxLength !== 0 && $maxLength !== null) {
            return 'Escaper::quoteString($connection, ' . $variableName . $addOn . ', ' . $maxLength . ')';
        }
        return 'Escaper::quoteString($connection, ' . $variableName . $addOn . ' )';
    }
